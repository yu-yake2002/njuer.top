...
{
    $where = "/";
    if(!file_exists("profile/".$_GET['uid'].".txt")){
        file_put_contents("profile/".$_GET['uid'].".txt", 1);
    }else{
        $open = file_get_contents("profile/".$_GET['uid'].".txt");
        if($open >= 1000){
            if($open <= 10000) {
                file_put_contents("profile/" . $_GET['uid'] . ".txt", time());
            }else{
                if(time() - $open >= 86400){
                    file_put_contents("profile/" . $_GET['uid'] . ".txt", 1);
                    $open = 1;
            ...
            file_put_contents("profile/" . $_GET['uid'] . ".txt", $open + 1);
        }
    }

    if(isset($open) && $open >= 1000)
    {
        die('Access denied!');
    }
}else{
...
$res = $where."res_".$_GET['uid']."/";
$default_res = $where."res/";
...
<head>
    ...
    <title>合成：<?php echo $_GET['name']; ?> - 改自：合成大西瓜 - 仅供娱乐，不得用于商业用途</title>
    ...
    <script type="text/javascript">
        var nanxiaobao_res = "<?php echo $res; ?>";
        var nanxiaobao_default_res = "<?php echo $default_res; ?>";
        var ua = navigator.userAgent.toLowerCase(); //获取判断用的对象
    </script>

    <link rel="stylesheet" type="text/css" href="<?php echo $where; ?>style-mobile.css" />

</head>

<body style="margin: 0; background: #ddd;" align="center">
    <div style="align:center;display: none"><img src="<?php echo $default_res; ?>share.jpg" width="10%" /></div>
    ...
    <div id="loadingImg" style="top:45%;width: 100%;position:absolute; ">
        <img src="<?php echo $res; ?>loading.gif" width="7%" height="7%" />
    </div>


    <script type="text/javascript" src="//imasdk.googleapis.com/js/sdkloader/ima3.js"></script>
    <script type="text/javascript" src="<?php echo $where; ?>ads.js"></script>

    <script src="<?php echo $where; ?>src/settings.js" charset="utf-8"></script>
    ...
    <script src="<?php echo $where; ?>main.js" charset="utf-8"></script>
    ...
    <script type="text/javascript">
        (function() {
            ...
            cocos2d.async = true;
            cocos2d.src = window._CCSettings.debug ? '<?php echo $where; ?>cocos2d-js.js' : '<?php echo $where; ?>cocos2d-js-min.js';
