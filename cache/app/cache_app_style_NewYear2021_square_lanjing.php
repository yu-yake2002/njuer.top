...
<?php include template("app_style/NewYear2021:common:header"); ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>南小宝 - 消息列表</title>
</head>
<body>
<?php include template("app_style/NewYear2021:square:common_header"); ?>
<div class="nav_row">
    ...
    <a href="index.php?mod=square&action=lanjing&do=Message"
       class="nav_btn<?php if($_GET['do'] == 'Message'){ ?>_chosen<?php } ?>">
        我的消息
    </a>
    <a href="index.php?mod=square&action=lanjing&do=Attention"
       class="nav_btn<?php if($_GET['do'] == 'Attention'){ ?>_chosen<?php } ?>">
        我的关注
    </a>
...
<div id="AttentionList">
    <?php while($attention = db_fetch($attentions)){ ?>
        <?php $avatar = db_fetch(db_query("SELECT avatar FROM user_profile WHERE uid = ".$attention['attention']))['avatar']; ?> 
         <?php $avatar = $avatar?$avatar:"data/avatar/common.png"; ?>
        <a href="index.php?mod=user&action=profile&uid=<?php echo isset($attention['attention'])?($attention['attention']):(""); ?>"><img src="<?php echo isset($avatar)?($avatar):(""); ?>" class="avatar_middle"
...
<?php if($_GET['do'] == 'Message'){ ?>
<script src="static/js/GetMessage.js?r=5434"></script>
<script>
    var ws;
    ...
    function onOpen(evt) {
        ws.send("<?php echo isset($_G['user']['uid'])?($_G['user']['uid']):(""); ?>,-2,<?php echo isset($token)?($token):(""); ?>");
        heartCheck.start();
    }
